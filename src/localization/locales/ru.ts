export default {
  'English': 'Английский',
  'Russian': 'Русский',
  'Application': 'Приложение',
  'Log In': 'Логин',
  'Sign Up': 'Регистрация',
  'Logout': 'Логаут',
  'Fitness Calculator': 'Фитнес Калькулятор',
  'Take control over your weight.': 'Возьмите под контроль свой вес.',
  'Get Started': 'Начнём',
  'Most Complete Product Database': 'Наиболее Полная База Продуктов',
  'We provide access to a complete database of products checked by our moderators.': 'Мы предоставляем доступ к полной базе данных продуктов, проверенных нашими модераторами.',
  'Most Full Statistic': 'Наиболее Полная Статистика',
  'We provide you with all the information you need to analyze and adjust your diet.': 'Мы предоставляем вам всю информацию, необходимую для анализа и корректировки вашей диеты.',
  'Check It Out': 'Убедитесь Сами',
  'Great application': 'Отличное приложение',
  'That is what our users say': 'Это то, что говорят наши пользователи',
  'It has helped me to become fit.': 'Это помогло мне обрести форму.',
  'Eugene, fitness newcomer': 'Евгений, фитнес новичок',
  'Stop Thinking, Start Doing': 'Хватит Думать, Начинай Действовать',
  'Instead of putting off the problem for tomorrow just start now.': 'Вместо того, чтобы откладывать проблему на завтра, начните прямо сейчас.',
  'Give A Try': 'Попробовать',
  'We give you a tool so you can change your life.': 'Мы даем вам инструмент, чтобы вы могли изменить свою жизнь.',
  'Submit': 'Подтвердить',
  'Name': 'Имя',
  'Email': 'Адрес электронной почты',
  'Password': 'Пароль',
  'Forgot password?': 'Забыли пароль?',
  'Password recovery': 'Восстановление пароля',
  'Enter email': 'Введите адрес электронной почты',
  'You will get a password recovery token': 'Вы получите токен восстановления пароля',
  'Enter password recovery token': 'Введите токен восстановления пароля',
  'The one that was sent to you by email': 'Тот, который был отправлен вам по электронной почте',
  'Change password': 'Поменяйте пароль',
  'Password recovery token': 'Токен восстановления пароля',
  'New password': 'Новый пароль',
  'Settings': 'Настройки',
  'Statistics': 'Статистика',
  'Products': 'Продукты',
  'Eating': 'Питание'
};

